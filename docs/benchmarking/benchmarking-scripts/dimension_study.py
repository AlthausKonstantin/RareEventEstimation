...
kwargs_dict = {
    ...
    "CBREE (vMFNM)": dict(callback=callback_vmfnm, observation_window=6),
    "CBREE (vMFNM, resampled)": dict(
        resample=True, mixture_model="vMFNM", divergence_check=False
    ),
}
solver_list = [
...
problem_list = [ree.make_fujita_rackwitz(d).set_sample(J, seed=J) for d in dim_list]
