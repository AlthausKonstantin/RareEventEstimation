...
def diffusion_term(x):
    return -1 * np.exp(np.sin(10 * x))


def rhs(x):
    return (
        2 * np.exp(np.sin(10 * x)) * (3 * x + 5 * (-2 + 3 * x) * x * np.cos(10 * x) - 1)
    )
...
def test_DiffusionPDE():
    ...
    problem.solve()
    assert np.amax(np.abs(u_true(np.linspace(0, 1, n)) - problem.solution)) < 1e-3
    return problem
...
def test_dirichlet_conditions():
    ...
    problem.solve()
    assert np.amax(np.abs(dirichlet_inhom - problem.solution[[0, -1]])) < 1e-10
    return problem
