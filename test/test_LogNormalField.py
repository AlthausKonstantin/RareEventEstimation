...
def test_LogNormalField():
    ...
    field = ree.LogNormalField(
        m, s, l, num_terms, np.random.default_rng(1))
    x = np.array([0.2, 0.5])
    # when
    num_samples = 5000
    ...
    for i in range(num_samples):
        samples[..., i] = field(x)
    # then
    cross_cov = np.exp(-np.abs(np.diff(x))/l)
    ...
    assert np.linalg.norm(
        np.cov(np.log(samples), rowvar=True)-true_cov, ord=np.inf) < 0.05, f"Error in Cov too large, {np.cov(np.log(samples), rowvar=True)} vs {true_cov}"
