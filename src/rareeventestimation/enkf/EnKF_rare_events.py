...
class EnKF_rare_events:
    ...
    def __init__(self, g, y_data, niter_enkf, ensemble_size, parameter_dim, tar_cov, space_dim, local_covariances,
                 g_original, k_init, mixture_model, gamma_weights=1, tar_cov_pf=0.05):
    ...
    def perform_tempering(self):
        ...
        c_up = EnKF_rare_events.calculate_c_up(self, self.uk, self.gk, self.ensemble_mean, self.lsf_mean)
        c_pp = EnKF_rare_events.calculate_c_pp(self, self.gk, self.lsf_mean)

        'sample the noise for the data'
        noise = np.random.normal(0, np.sqrt(1 / self.delta_sigma), (self.data_dim, self.ensemble_size))

        if self.data_dim == 1:
            y_noisy = self.y_data + noise.T
            self.uk = self.uk + np.outer(c_up, 1/(c_pp + 1/self.delta_sigma) * (y_noisy - self. gk)).T
        else:
            'Update the parameter vector u'
            for j in range(0, self.ensemble_size):
                y_noisy = self.y_data + noise[:, j]
                m = np.linalg.solve(c_pp + 1 / self.delta_sigma * np.identity(self.data_dim), y_noisy - self.gk[j])
    ...
    def perform_tempering_local(self):
        ...
        if self.data_dim == 1:
            ...
            for j in range(self.ensemble_size):
                ...
                c_up_loc[j, :] = np.matmul(w_mat[j, :], (self.uk - weighted_mean[j]) * diff)
            y_noisy = self.y_data + noise.T
            self.uk = self.uk + np.multiply(c_up_loc, 1 / (c_pp_loc + 1 / self.delta_sigma) * (y_noisy - self.gk))
            ...
            for j in range(self.ensemble_size):
                ...
                for i in range(self.ensemble_size):
                    c_pp_loc = c_pp_loc + w_mat[j, i] * np.outer(self.gk[i] - weighted_lsf_mean[j], self.gk[i] - weighted_lsf_mean[j])
                    c_up_loc = c_up_loc + w_mat[j, i] * np.outer(self.uk[i] - weighted_mean[j], self.gk[i] - weighted_lsf_mean[j])
                y_noisy = self.y_data + noise[:, j]
                m = np.linalg.solve(c_pp_loc + 1 / self.delta_sigma * np.identity(self.data_dim), y_noisy - self.gk[j])
    ...
    def calculate_c_up(self, u, g, ensemble_mean, lsf_mean):
        ...
        if self.data_dim == 1:
            ...
            for i in range(0, ensemble_size):
                c_up = c_up + np.matmul((u[i] - ensemble_mean).transpose(), (g[i] - lsf_mean))
    ...
    def calculate_c_pp(self, g, lsf_mean):
        ...
        if self.data_dim == 1:
            ...
            for i in range(0, ensemble_size):
                c_pp = c_pp + np.matmul((g[i] - lsf_mean).transpose(), (g[i] - lsf_mean))
    ...
    def update_g_and_sigma(self):
        ...
        if self.iter == 0:
            ...
            self.sigmak_list[self.iter] = fminbound(func_0, 0, 10*gmu, args=(self.y_data, self.gk, self.tar_cov), xtol=1e-20)
            ...
            def func(x, sigma, y_data, gk, tar_cov):
                w = np.exp(-(1/x - 1/sigma) / 2 * np.linalg.norm(y_data - gk, axis=1) ** 2)
                return (np.std(w) / np.mean(w) - tar_cov) ** 2

            self.sigmak_list[self.iter] = fminbound(func=func, x1=0, x2=self.sigmak_list[self.iter - 1],
                                                    args=(self.sigmak_list[self.iter - 1], self.y_data, self.gk, self.tar_cov),xtol=1e-20)

            self.delta_sigma = 1/self.sigmak_list[self.iter] - 1/self.sigmak_list[self.iter - 1]
    ...
    def update_posterior_and_error(self):
        ...
        self.rel_error.append(np.linalg.norm(self.y_data - self.gk) / self.ensemble_size)

        if np.min(self.gk) == 0:
            wk_opt = np.zeros((self.ensemble_size,1))
            wk_opt[self.gk == 0] = np.exp(1/
                self.sigmak_list[self.iter] / 2 * np.linalg.norm(self.y_data - self.gk[self.gk == 0], axis=0) ** 2)
    ...
    def calculate_failure_probability_GM(self):
        ...
        wk = (self.gk_fitted<0) * multivariate_normal.pdf(self.uk_fitted, mean=np.zeros(self.parameter_dim), cov=np.eye(self.parameter_dim))/\
    ...
    def calculate_failure_probability_vMFNM(self):
        ...
        wk = (self.gk_fitted < 0) * np.exp(likelihood_ratio_log(self.uk_fitted, mu.T, kappa, omega, m, alpha)).flatten()
