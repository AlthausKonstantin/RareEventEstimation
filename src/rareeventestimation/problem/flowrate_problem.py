...
def make_flowrate_problem(d: int,
                          num_gridpoints: int,
                          mean_gaussian=0.1,
                          variance_gaussian=0.04,
                          correlation_length=0.3,
                          dirichlet_conditions=[1, 0]) -> NormalProblem:
    ...
    def lsf(sample: ndarray) -> Real:
        ...
        flow = - random_field(ones(1), noise=sample) * \
            diffuison_pde.solution_grad[-1]
...
class DiffusionPDE:
    ...
    def __init__(self, dirichlet_conditons: ndarray,
                 diffusion_term: Callable,
                 rhs: Callable,
                 num_gridpoints: int) -> None:
    ...
    def solve(self) -> None:
        ...
        def dirichlet(x):
            return (1 - x[0])*self.dirichlet_conditions[0] + x[0]*self.dirichlet_conditions[1]
        ...
        self.solution = basis.with_element(
            fem.ElementLineP1()).project(basis.interpolate(u))
        self.solution_grad = basis.with_element(
            fem.ElementLineP1()).project(basis.interpolate(u).grad[0])
