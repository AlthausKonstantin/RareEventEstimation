from numpy import eye, ones, sum, diff, amin, sin, log, exp, array, sqrt, abs
...
# Convex-Problem
prob_dim = 2
def lsf_convex(x): return 0.1*(x[..., 0]-x[..., 1]
                               )**2 - 1/sqrt(2)*sum(x, axis=-1)+2.5
...
prob_nonlin_osc = NormalProblem(
    lsf_nonlin_osc, 6, 1, prob_fail_true=6.43*1e-6, name="Nonlinear Oscillator Problem")
